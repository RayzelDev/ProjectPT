		...
		if (lpChar->MotionInfo->State != CHRMOTION_STATE_DEAD && lpChar->MotionInfo->State != CHRMOTION_STATE_WARP) {
			...
			if (lpChar->smCharInfo.State) {
				...
				if (cnt > 0) {
					...
					if (lpChar->lpAttackDamageList || lpChar->lpAttackDamageList_BlessCastle || 1) rsRecordCharDamage(lpPlayInfo, lpChar, cnt);	//¹ÞÀº°ø°Ý ±â¾ïÇÏ´Â ¸÷
...
///////////////////////// µð¹ö±×¿ë µ¥¹ÌÁö Àû¿ë Ãâ·Â //////////////////////
						...
						if (pow > 0) {
							...
							if (lpChar->lpAttackDamageList || lpChar->lpAttackDamageList_BlessCastle || 1) rsRecordCharDamage(lpPlayInfo, lpChar, pow);	//¹ÞÀº°ø°Ý ±â¾ïÇÏ´Â ¸÷
				...
				for (cnt = 0; cnt < lpTransSkilAttackData->TargetCount; cnt++) {
					...
					if (lpChar->dwObjectSerial == lpTransSkilAttackData->dwTarObjectSerial[cnt] &&
						...
						if (pow > 0) {
							...
							SendShowDmg(lpChar, pow, 6, lpPlayInfo);
							if (lpChar->lpAttackDamageList || lpChar->lpAttackDamageList_BlessCastle || 1) rsRecordCharDamage(lpPlayInfo, lpChar, life);	//¹ÞÀº°ø°Ý ±â¾ïÇÏ´Â ¸÷
