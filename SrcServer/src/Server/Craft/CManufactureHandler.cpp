...
void CManufactureHandler::SendOpenManufactureWindow(rsPLAYINFO* lpPlayInfo)
{
	...
	PacketOpenManufactureWindow sPacket;
	sPacket.size = sizeof(sPacket);
	sPacket.code = smTRANSCODE_OPEN_NEWMANUFACTURE;
...
void CManufactureHandler::HandlePacket(rsPLAYINFO* lpPlayInfo, PacketGetRecipe* psPacket)
{
	...
	if (db->Open())
	{
		if (db->Prepare("SELECT  Slot1, Slot2, Slot3, cItem1, cItem2, cItem3, cItem4, cItem5, cItem6, cItem7, cItem8, cItem9, cItem10, cItem11, Chance,ChanceF, Price FROM CraftRecipe1 Where RecipeCode = ?"))
		{
			...
			if (db->Execute())
			{
				...
				if (!sPacket.StoneCode[0][0])
				{
					...
					db->GetData(&sPacket.ChanceF, 4, index, PARAMTYPE_Integer);
...
void CManufactureHandler::HandlePacket(rsPLAYINFO* lpPlayInfo, PacketManufactureRecipe* psPacket)
{
		...
		if (db->Prepare("SELECT  Slot1, Slot2, Slot3, cItem1, cItem2, cItem3, cItem4, cItem5, cItem6, cItem7, cItem8, cItem9, cItem10, cItem11, Chance,ChanceF, Price FROM CraftRecipe1 Where RecipeCode = ?"))
		{
			...
			if (db->Execute())
			{
				if (!StoneCode[0][0])
				{
					...
					db->GetData(&ChanceF, 4, index, PARAMTYPE_Integer);
	...
	if (Price > NewShop::GetInstance()->GetPlayerCoin(lpPlayInfo))
	{
		SendQuickMessage2(lpPlayInfo, "Você não tem Coins suficiente.");
		return;
	}
	else
		NewShop::GetInstance()->removePlayerCoins(lpPlayInfo, Price);
		...
		if (Chance < 100)
		{
			if ((rand() % 100) > Chance)
			{
				...
				if (srSetItemFromCode(&psItem, temp))
				{
					...
					RecordLog1("CraftItem", FormatString("Player[%s] falhou no Craft de %s", lpPlayInfo->szName, psItem.ItemInfo.ItemName));
					return;
				}
			}
		}
	...
	if (vItemCodes.size())
	{
		...
		if (srSetItemFromCode(&psItem, temp))
		{
			...
			wsprintf(sPacket.szMessage, "Craft: %s craftou com sucesso %s!", lpPlayInfo->szName, psItem.ItemInfo.ItemName);
			...
			RecordLog1("CraftItem", FormatString("[ %s ] craftou com sucesso [ %s ]!", lpPlayInfo->szName, psItem.ItemInfo.ItemName));
		}
