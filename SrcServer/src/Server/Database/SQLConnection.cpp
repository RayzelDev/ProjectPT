...
bool SQLConnection::Init(EDatabaseID eDatabaseID, SQLInstanceRead instance)
{
	...
	SQLCHAR strconn[512];
	std::memset(szStringConnectionSQL, 0, sizeof(szStringConnectionSQL));


	/*STRINGFORMAT(szStringConnectionSQL, "Persist Security Info=False; Integrated Security=False; Driver={ODBC Driver for SQL Server}; Server=%s; Database=%s; Uid=%s; Pwd=%s;", instance.szHost, szDatabaseName, instance.szUser, instance.szPassword);*/


	//correçao para fica compativel qualquer versao do odbc
	std::stringstream ss;
	ss << "Persist Security Info=False; Integrated Security=False; Driver=" << "{SQL Server}"
		<< "; Server=" << instance.szHost
		<< "; Database=" << szDatabaseName
		<< "; Uid=" << instance.szUser
		<< "; Pwd=" << instance.szPassword << ";";
	std::string str = ss.str();

	std::strncpy(szStringConnectionSQL, str.c_str(), sizeof(szStringConnectionSQL));
	szStringConnectionSQL[sizeof(szStringConnectionSQL) - 1] = 0;  // Garante que a string é null-terminated



	std::memcpy(strconn, szStringConnectionSQL, sizeof(szStringConnectionSQL));
...
BOOL SQLConnection::GetData(void* pParameter, int iSize, int iPosition, EDatabaseDataType eDataType)
{
	int iType = 0;
