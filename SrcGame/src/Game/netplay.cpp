...
char* szDefaultServIP = "189.46.228.170";
...
int rsTRANS_SERVER::RecvMessage(smTHREADSOCK* pData)
{
	...
	case OPCODE_ROLLDICE_END:
		RollDice::GetInstance().HandlePacket(IOPacket::PacketReader{ pData->Buff, IPData[0] }, IPData[1]);
		break;
	...
	case 0x44550005: // OURO INICIAL
		sinPlusMoney(1000000);
		ReformCharForm();
		AddInvenMoney(1000000);
	...
	case smTRANSCODE_WHISPERMESSAGE:
		...
		{
			global = true;
			AlertaDrop[strlen(lpTransChatMessage->szMessage)];
			strcpy(AlertaDrop, lpTransChatMessage->szMessage);
		}
		break;
	case 0x48470024:
		lpTransCommand = (smTRANS_COMMAND*)pData->Buff;
	...
	case 0x48480066:
	{
		...
		if (lpTransCommand->LParam)
		{
			...
			{
				if (sSkill[j].CODE == VIPPLAYER)
				{
					std::memcpy(&haSkill, &sSkill[j], sizeof(sSKILL));
					haSkill.UseTime = 604800;
					sinContinueSkillSet(&haSkill);
					bVip = TRUE;
					break;
				}
			}
	...
	case 0x48470027: // select top lvl server xxstr
	{
		...
		if (smTransCommand->SParam && !bTopLVL)
		{
...
*/
	...
	case OPCODE_OPEN_REVIEWSKIN:
		REVIEWSKINWINDOW->Open();

		break;
	case OPCODE_GET_REVIEWSKIN:
		REVIEWSKINWINDOW->HandlePacket((PacketReviewSkin*)pData->Buff);
		break;


	case smTRANSCODE_OPEN_NEWMANUFACTURE:
		CRAFTWINDOW->HandlePacket((PacketOpenManufactureWindow*)pData->Buff);
		break;

	case smTRANSCODE_GET_RECIPE:
		CRAFTWINDOW->HandlePacket((PacketRecipeInfo*)pData->Buff);
		break;

	case smTRANSCODE_OPEN_MANUFACTUREGOLD:
		CRAFTGOLDWINDOW->HandlePacket((PacketOpenManufactureGoldWindow*)pData->Buff);
		break;

	case smTRANSCODE_GET_RECIPEGOLD:
		CRAFTGOLDWINDOW->HandlePacket((PacketRecipeInfoGold*)pData->Buff);
		break;
...
int funcValeLevel(int LevelToGo, DWORD ItemCode, DWORD ItemHead, DWORD ItemChkSum)
{
	...
	if (LevelToGo == 2 && (sinChar->Level < 120 || sinChar->Level > 134)) flag = 1;
	if (LevelToGo == 3 && (sinChar->Level < 130 || sinChar->Level > 139)) flag = 1;
	if (LevelToGo == 4 && (sinChar->Level < 140 || sinChar->Level > 144)) flag = 1;
