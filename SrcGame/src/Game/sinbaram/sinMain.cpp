...
void sinInit() // na sua deve ta faltado referencia que vai nessa parte
{
	InitSub();
	InitCursor(); 
	cItem.Init();
	...
	clanMENU_INIT(1);
	DrawHPBOSS->Init();//extremez
	TCORE::GetInstance()->init(); // eo modo ele gera a imagem da interface que ta pobrema tem olha o codigo vc copia  tema lgo diferent 
	CRAFTWINDOW->Init(); // parte responsavel ele  inicia   o erro vai ta em uma dessa duas  pois nao e na parte do servidor  ou jogo tinha crashado
	CRAFTGOLDWINDOW->Init();
	RollDice::GetInstance().Initialize();
...
void sinDraw()
{
	...
	REVIEWSKINWINDOW->Draw();
	cSkinChanger.Draw();
	CRAFTWINDOW->Render(); // vc vai ter olha toda essa parte aki na que vc copio esse dev pois essa responsavel puxa interface e butoues
	CRAFTGOLDWINDOW->Render();
	RollDice::GetInstance().Render();
	cItem.Draw();
...
void sinProc(int Message)
{
	...
	case SINLBUTTONDOWN:
		...
		if (CRAFTWINDOW->OnMouseClick(true)) break;
		if (CRAFTGOLDWINDOW->OnMouseClick(true)) break;
		if (REVIEWSKINWINDOW->OnMouseClick(true)) break;
		if (RollDice::GetInstance().LButtonDown()) break;
	...
	case SINLBUTTONUP:
		if (sinbaram_Stop)return;
		CRAFTWINDOW->OnMouseClick(false);
		CRAFTGOLDWINDOW->OnMouseClick(false);
		REVIEWSKINWINDOW->OnMouseClick(false);
