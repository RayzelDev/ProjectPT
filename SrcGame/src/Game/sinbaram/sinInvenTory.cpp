		...
		if (SelectInvenItemIndex)
		{
			...
			else if ((InvenItem[Index].CODE & sinITEM_MASK2) == sinOS1) //COLOCAR A PEDRA
			{
				if (pRecipe->Flag)
				{
					...
					for (size_t i = 0; i < 3; i++)
					{
						auto pItemFrame = janelaCraftGold->GetElement<UINewItemFrame>(Craft_Slots + i); //Verifica quantas runas/pedras já tem
						if (pItemFrame)
						{
							auto pItem = pItemFrame->getItem();
							if (pItem->Flag)
...
int cINVENTORY::ThrowInvenItemToField(sITEM* pItem, int SellRight)
{
	...
	if (cTrade.OpenFlag || cWareHouse.OpenFlag || cCraftItem.OpenFlag || cAging.OpenFlag || SmeltingItem.OpenFlag || CRAFTWINDOW->isOpen() || CRAFTGOLDWINDOW->isOpen() || ManufactureItem.m_OpenFlag || cMixtureReset.OpenFlag || Caravana::GetInstance()->OpenFlag)		//Æ®·¹ÀÌµå³ª Ã¢°í°¡ ¶°ÀÖÀ¸¸é ¾ÆÀÌÅÛÀ» ¹ö¸®Áö ¾Ê´Â´Ù  // ¼®Áö¿ë - ¹Í½ºÃÄ ¸®¼Â
		return TRUE;
