...
UINewImage::UINewImage(int iID, char * path, int x, int y, int w, int h, std::function<void()> hover)
{
	setID(iID);
	setX(x);
	setY(y);
...
void UINewImage::Render(int iX, int iY, int iWidth, int iHeight, int iSourceX, int iSourceY)
{
	if (isVisible())
	{
		...
		{
			DrawSprite(iRenderX, iRenderY, Image, iStartX + startX, iStartY + startY, iRenderWidth + startX, iRenderHeight + startY);
...
void UINewImage::SetImage(char * path)
{
	Image = LoadUIImage(path);
