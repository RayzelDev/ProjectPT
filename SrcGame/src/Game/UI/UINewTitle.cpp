...
UINewTitle::UINewTitle(int iID, char* left, char* middle, char* right, int x, int y, char* text, bool bold, DWORD dwColor)
{
	setID(iID);
	setX(x);
	setY(y);

	Left = new UINewImage(0, left, 0, 0);
	Middle = new UINewImage(0, middle, 0, 0);
	Right = new UINewImage(0, right, 0, 0);
	Text = new UINewText(0, text, 0, 0, bold, dwColor);
...
void UINewTitle::Render(int iX, int iY, int iWidth, int iHeight, int iSourceX, int iSourceY)
{
	if (isVisible())
	{
...
void UINewTitle::setText(string text)
{
	Text->setText(text);
	Middle->setWidth(Text->getWidth());
	Middle->setX(Left->getWidth());
