...
void CFont::GetTextSize1(const char* lpString, SIZE* lpSize, int x, int len)
{
	if (!Font)
		return;

	RECT rcRect = { 0,0,x,0 };
	Font->DrawText(NULL, lpString, len ? len : lstrlen(lpString), &rcRect, DT_CALCRECT | (x ? DT_WORDBREAK : 0), FontColor);
...
void CFont::DrawRect1(RECT rc, const char* lpszString, int nCount, UINT uFormat)
{
	Font->DrawText(NULL, lpszString, lstrlen(lpszString), &rc, uFormat, FontColor);
}
