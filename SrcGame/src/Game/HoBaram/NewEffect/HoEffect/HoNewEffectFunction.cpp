...
// effect top lvl xxstr
void Effect_VipPlayer(smCHAR* pChar)
{
	if (pChar)
	{

		if (HoEffectManager::GetInstance()->SetMaintainEffect(VIPPLAYER, pChar, 0.f))
			return;
		else
		{
			HoEffectGroup* effectGroup = NULL;
			effectGroup = PARTICLEFACTORY->LoadScript("Effect\\NewEffect\\VIPPLAYER.lua");
			if (effectGroup)
			{
				HoEffectType_Attach_Avater* effectType = new HoEffectType_Attach_Avater();
				effectType->Init(pChar, effectGroup, 300, 0.f);
				...
				int num = g_NewParticleMgr.Start("VIPPLAYER", pos);
				effectType->SetParticleID(num);
				effectType->SetSkillCode(VIPPLAYER);
				HoEffectManager::GetInstance()->AddMaintainEffect(effectType);
			}
		}
	}
